<template>
  <div class="canada28-page">
    ...
    <div class="page-header">
      ...
      <h1 class="page-title">{{ $t('game.canadaHighTitle') }}</h1>
    ...
    <div class="tab-content">
      ...
      <div v-if="activeTab === 'bet'" class="bet-area">
        ...
        <div class="bet-content">
          ...
          <!-- Double-sided positioning (shuangmian) -->
          <template v-if="activeSidebar === 'shuangmian'">
            <div class="bet-section">
              <h4 class="section-title">{{ $t('game.bigSmallOddEven') }}</h4>
              <div class="bet-grid grid-4">
                <button
                  ...
                  :key="'sm-' + item.label"
                  class="bet-btn"
                  :class="{ selected: isSelected('sm_' + item.id) }"
                  @click="toggleBet({ ...item, id: 'sm_' + item.id })"
                >
                  <span class="bet-label">{{ item.label }}</span>
                  <span class="bet-odds">{{ item.odds }}</span>
                </button>
          ...
          <!-- Tail (weixiang) -->
          <template v-if="activeSidebar === 'weixiang'">
            <div class="bet-section">
              <h4 class="section-title">{{ $t('game.tailBet') }}</h4>
              <div class="bet-grid grid-4">
                <button
                  v-for="item in weixiangItems"
                  :key="item.label"
                  class="bet-btn"
                  :class="{ selected: isSelected(item.id) }"
                  @click="toggleBet(item)"
          ...
          <!-- Dragon Tiger (longhu) -->
          <template v-if="activeSidebar === 'longhu'">
            <div class="bet-section">
              <h4 class="section-title">{{ $t('game.dragonTigerTie') }}</h4>
              <div class="bet-grid grid-3">
                <button
                  v-for="item in longhuItems"
                  :key="item.label"
                  class="bet-btn"
                  :class="{ selected: isSelected(item.id) }"
                  @click="toggleBet(item)"
      ...
      <div v-if="activeTab === 'rules'" class="rules-content">
        <div class="rule-block">
          ...
          <p>{{ $t('game.canadaHighDescText') }}</p>
        </div>
        <div class="rule-block">
          <h4>{{ $t('game.bigSmallOddEven') }}</h4>
          <p>{{ $t('game.canadaHighBsoeRule') }}</p>
        </div>
        <div class="rule-block">
          <h4>{{ $t('game.combination') }}</h4>
          <p>{{ $t('game.canadaHighComboRule') }}</p>
        </div>
        <div class="rule-block">
          <h4>{{ $t('game.extremeBigSmall') }}</h4>
          <p>{{ $t('game.canadaHighExtremeRule') }}</p>
        </div>
        <div class="rule-block">
          <h4>{{ $t('game.tailBet') }}</h4>
          <p>{{ $t('game.canadaHighTailRule') }}</p>
        </div>
        <div class="rule-block">
          <h4>{{ $t('game.dragonTigerTie') }}</h4>
          <p>{{ $t('game.canadaHighDragonRule') }}</p>
        </div>
      </div>
    </div>
...
const sidebarItems = computed(() => [
  ...
  { key: 'shuangmian', label: t('game.doubleSided') },
  { key: 'sanwei', label: t('game.threeDigit') },
  { key: 'dandian', label: t('game.singlePoint') },
  { key: 'weixiang', label: t('game.tailBet') },
  { key: 'longhu', label: t('game.dragonTigerTie') }
])
const activeSidebar = ref('tema')

// Mock data - different period seed
const currentPeriod = ref(5821034)
const nextPeriod = ref(5821035)
const resultBalls = ref([8, 3, 5])
const resultSum = computed(() => resultBalls.value.reduce((a, b) => a + b, 0))
const showHistory = ref(false)

const historyResults = [
  { period: 5821033, balls: [7, 2, 9], sum: 18 },
  { period: 5821032, balls: [1, 8, 6], sum: 15 },
  { period: 5821031, balls: [4, 5, 3], sum: 12 },
  { period: 5821030, balls: [9, 0, 7], sum: 16 },
  { period: 5821029, balls: [2, 6, 8], sum: 16 }
]

// Countdown - 3.5 minutes = 210 seconds
const countdown = ref(210)
let timer = null
...
// Bet options - higher odds
const daxiaodanshuang = computed(() => [
  { id: 'da', label: t('game.big'), odds: 1.98 },
  { id: 'xiao', label: t('game.small'), odds: 1.98 },
  { id: 'dan', label: t('game.odd'), odds: 1.98 },
  { id: 'shuang', label: t('game.even'), odds: 1.98 }
])

const zuhe = computed(() => [
  { id: 'dadan', label: t('game.bigOdd'), odds: 4.28 },
  { id: 'dashuang', label: t('game.bigEven'), odds: 3.68 },
  { id: 'xiaodan', label: t('game.smallOdd'), odds: 3.68 },
  { id: 'xiaoshuang', label: t('game.smallEven'), odds: 4.28 }
])

const jidaxiao = computed(() => [
  { id: 'jida', label: t('game.extremeBig'), odds: 16 },
  { id: 'jixiao', label: t('game.extremeSmall'), odds: 16 }
])
...
// Tail bet items
const weixiangItems = computed(() => [
  { id: 'weida', label: t('game.tailBig'), odds: 1.98 },
  { id: 'weixiao', label: t('game.tailSmall'), odds: 1.98 },
  { id: 'weidan', label: t('game.tailOdd'), odds: 1.98 },
  { id: 'weishuang', label: t('game.tailEven'), odds: 1.98 }
])

// Dragon Tiger Tie items
const longhuItems = computed(() => [
  { id: 'long', label: t('game.dragon'), odds: 2.0 },
  { id: 'hu', label: t('game.tiger'), odds: 2.0 },
  { id: 'he', label: t('game.tie'), odds: 58 }
])
